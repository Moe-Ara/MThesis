...
When FastAPI starts, a single \texttt{\_build\_services()} routine selects scorer/planner backends from environment or active registry profile and instantiates them once. This startup-time loading is deliberate: missing model files and invalid configurations fail fast at boot instead of appearing as runtime 500 errors.
...
The MLP receives a fixed-length feature vector assembled from alert metadata and assessment context: alert-type encoding, normalized severity/confidence, entity-presence flags, confidence/severity buckets, interaction terms, security keyword indicators, SIEM-source indicators, ATT\&CK-presence flag, and TF-IDF text features from hypothesis/evidence.`r`n`r`nThe network uses a shared encoder (\texttt{198 $\rightarrow$ 512 $\rightarrow$ 256 $\rightarrow$ 128}, with LayerNorm, GELU, and dropout) followed by three heads: strategy classification (5-way softmax), action-set prediction (8-way sigmoid), and priority regression (sigmoid scaled to 0--100). The final parameter count is approximately 270,000, far smaller than seq2seq alternatives.
